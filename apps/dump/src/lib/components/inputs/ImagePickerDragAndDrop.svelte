<script lang="ts">
	...
	onMount(() => {
		...
		fileInput.onmouseenter = () => {
			if (!loading) {
				if (!file)
					(fileInput.childNodes[0] as HTMLElement).style.opacity = '1'
				else {
					;(fileInput.childNodes[2] as HTMLElement).style.opacity =
						'1'
				}
			}
		}
		...
		fileInput.onmouseleave = () => {
			if (!loading) {
				if (!file)
					(fileInput.childNodes[0] as HTMLElement).style.opacity =
						'0.5'
				else {
					;(fileInput.childNodes[2] as HTMLElement).style.opacity =
						'0'
...
<div
	...
	{#if loading}
		<div
			class="card-body bg-base-100 items-center opacity-100 py-40 transition-all">
			<Spinner />
		</div>
	{:else if !validFile}
	...
	{:else if dataUrl}
		<figure class="p-4">
			<img
				class="card-img max-h-[70vh]"
				src={dataUrl}
				alt="uploaded file" />
		</figure>
		<div
			class="card-body items-center justify-center opacity-100 transition-all">
			<span class="text-6xl">
				<span class="material-icons-outlined"> file_upload </span>
			</span>
			<h1 class="text-xl">
