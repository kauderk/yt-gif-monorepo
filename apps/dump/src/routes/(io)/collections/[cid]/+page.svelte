	...
	<div>
		<div class="dropdown ">
			...
			<ul
				...
				{#if isAuthor}
					<li>
						<btn class="text-error" on:click={() => deletePost()}
							>delete</btn>
					</li>
				{/if}
			</ul>
			...
			<div class="card-body">
				<div class="flex flex-row justify-between gap-4">
					<div class="flex flex-row justify-start gap-4">
						...
						<div
							class="card-title flex-col justify-start items-start gap-0 font-normal h-fit">
							<p>{collection.name}</p>
							<p class="text-sm">
								{new Date(
				...
				<span class="kbd w-fit"
					>{collection.privacy.toLowerCase()}</span>
			</div>
		</div>
		{#if isAuthor}
