...
<div class="h-full flex flex-col justify-start items-center px-2">
	...
	<div bind:this={sectionsContainer} class="w-full shrink-0 grow">
					...
					<div class="form-control ">
						{#if notAddedMetatypes.length > 0}
							<div class="input-group ">
								...
								<button class="btn" on:click={addMetatype}
									>Add</button>
							</div>
						{/if}
					</div>
		...
		<section style="display: none;">
			<div
				class="card flex-shrink-0 w-full max-w-5xl shadow-2xl bg-base-300 m-4">
				<div class="hero bg-base-300 p-4">
					<div class="hero-content flex-col lg:flex-row">
						{#if dataUrl}
							<img
								src={dataUrl}
								class="md:max-w-sm shadow-2xl"
								alt="to be uploaded" />
						{/if}
						<div>
							<h1
								class="text-5xl font-bold overflow-ellipsis break-all">
								{title}
							</h1>
