...
export function tryFmt_timestamp({ page, value, match }: Tfmt_tm) {
	...
	const fmt =
		UIStore.get().timestamps.tm_workflow_grab.value != 'S'
			? 'hmsSufix'
			: 'S'
...
export function fmtUrlsObj(
	...
	const c = isSelected(UIStore.get().display.fmt_options, 'avoid_redundancy')
		? '/'
		: 'https://youtu.be/'
...
export function TryToRemoveRedundantTmParam(
	...
	if (
		rawValue === value &&
		isSelected(UIStore.get().display.fmt_options, 'avoid_redundancy')
	) {
		if (pear == 'self')
