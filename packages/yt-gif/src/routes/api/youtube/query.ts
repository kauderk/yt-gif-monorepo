...
export const setEnabledConsoleLog = b => (eneabledLog = b)
...
const GetData = async (
	...
	let endpoint =
		await `${youtubeEndpoint}/results?search_query=${keyword}&${_locale_}`
	try {
...
const nextPage = async (nextPage, withPlaylist = false, limit = 0) => {
	const endpoint =
		await `${youtubeEndpoint}/youtubei/v1/search?key=${nextPage.nextPageToken}&${_locale_}`
	try {
		...
		const item1 =
			page.data.onResponseReceivedCommands[0]
...
const GetPlaylistData = async (playlistId, limit = 0) => {
	const endpoint =
		await `${youtubeEndpoint}/playlist?list=${playlistId}&${_locale_}`
	try {
...
const GetSuggestData = async (limit = 0) => {
	const endpoint = await `${youtubeEndpoint}?${_locale_}`
	try {
		const page = await GetYoutubeInitData(endpoint)
		const sectionListRenderer = await page.initdata.contents
			.twoColumnBrowseResultsRenderer.tabs[0].tabRenderer.content
...
const GetChannelById = async channelId => {
	const endpoint = await `${youtubeEndpoint}/channel/${channelId}&${_locale_}`
	try {
		const page = await GetYoutubeInitData(endpoint)
...
const GetVideoDetails = async videoId => {
	const endpoint = await `${youtubeEndpoint}/watch?v=${videoId}&${_locale_}`
	try {
		const page = await GetYoutubeInitData(endpoint)
