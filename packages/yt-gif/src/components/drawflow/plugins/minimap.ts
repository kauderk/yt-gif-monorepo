...
export class DrawflowMinimap {
	minimap: typeof DrawflowClone
	constructor(container, editor, zoom) {
		...
		this.editor.on('translate', pos => {
			const editorWidth =
				this.editor.container.getBoundingClientRect().width
			const editorHeight =
				this.editor.container.getBoundingClientRect().height

			const minimapWidth =
				this.minimap.container.getBoundingClientRect().width
			const minimapHeight =
	...
	updateMask() {
		...
		this.mask.style.top = `${
			(minimapHeight -
				((editorHeight - minimapHeight) / this.editor.zoom) *
					this.zoom) /
			2
		}px`
	}
