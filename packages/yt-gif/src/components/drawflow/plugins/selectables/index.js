 ...
 *
 *   v1.4.1
 *   copied from https://github.com/p34eu/selectables/blob/master/selectables.js, beacuse it isn't an ESM
 *
...
export default function Selectables(opts) {
	...
	var defaults = {
		...
		hoverClass: 'hover', // class name to apply to seleted items
	...
	this.select = function (e) {
		...
		self.foreach(self.items, el => {
			el.classList.remove(self.options.hoverClass)
		})
	...
	this.rectDraw = function (e) {
		...
		var s = self.options.hoverClass
		self.foreach(self.items, el => {
			if (cross(g, el) === true) {
				el.classList.add(s)
				self.options.onmouseenter?.(el)
			} else {
				el.classList.remove(s)
				self.options.onmouseleave?.(el)
			}
		})
	}
