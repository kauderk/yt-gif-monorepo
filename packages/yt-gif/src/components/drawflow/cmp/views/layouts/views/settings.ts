...
export const SettingsSchema = {
	...
	search: {
		Preserve: [
			Kind.Multi,
			['Category', 'Query', 'Category + Query', 'None'],
		],
	},
	about: {
...
function _settings() {
	...
	if ('localStorage' in self === false) return writable(list)
	...
	if (!stored?.appearance && !stored?.playback && !stored?.search) {
		...
		list.playback['Prefer WebM Audio'] =
			...
			) as Playback['Prefer WebM Audio']) ??
			list.playback['Prefer WebM Audio']
		list.playback['Dedupe Automix'] =
			(Boolean(
				localStorage.getItem('theme')
			) as Playback['Dedupe Automix']) ?? list.playback['Dedupe Automix']
		localStorage.clear()
