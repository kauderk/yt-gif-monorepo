		...
		if (this.connection === true) {
			if (
				...
				if (
					...
					if (
						Object.keys(
							this.getNodeFromId(input_id.slice(5)).inputs
						).length === 0
					) {
			...
			this.drawflow.drawflow[moduleName].data[itemx.node].outputs[
				...
				if (itemz.node == id) {
					...
					if (parseInt(input_class_id) < parseInt(output_id)) {
						...
						if (itemz.points) {
							this.drawflow.drawflow[moduleName].data[
								itemx.node
							].outputs[itemx.input].connections[g] = {
								node: itemz.node,
				...
				if (itemz.node == id) {
					...
					if (parseInt(output_class_id) < parseInt(input_id)) {
						...
						if (itemz.points) {
							this.drawflow.drawflow[moduleName].data[
								itemx.node
							].inputs[itemx.output].connections[g] = {
								node: itemz.node,
